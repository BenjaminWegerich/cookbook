...
- Display examples:
  ...
  - Stored: "15 ml Zitronensaft" → displayed as "15 ml Zitronensaft (½ Zitrone)", if the conversion table defines ½ Zitrone = 15 ml.
- Display conventions:
  - The additional unit is shown only if a fitting conversion exists for that ingredient and quantity; otherwise the base amount alone is shown.
  - The order, as well as punctuation (e.g., parentheses), is specified for each additional unit.
