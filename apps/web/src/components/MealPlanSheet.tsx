/**
 * Meal-plan overlay: puts a known recipe on the Google Keep meal plan.
 *
 * Opened from the recipe overview's "Einplanen" action (a known recipe that is
 * not on the meal plan yet). The overlay asks for the size to cook and nothing
 * else:
 *
 ...
 *   ladder stepper) for an ingredient recipe — and it starts on the size the
 *   recipe is written in (its servings / its yield);
 ...
 *   button, while the button reports the running write ("Wird hinzugefügt …",
 *   the shared busy label).
 *
 ...
 * and its own backdrop closes just this layer.
 *
...
interface MealPlanSheetProps {
   ...
   * Resolves when the dish is on the plan — the caller then closes the flow —
   * and rejects with the reason when it failed.
   */
...
function MealPlanSheet({ recipe, exportUrl, onClose, onConfirm }: MealPlanSheetProps) {
  ...
  /** Selected serving count (finished dish); starts on the written size. */
  const [servings, setServings] = useState(recipe.servings ?? 1);
  /** Selected yield in the family unit (ingredient recipe); starts on the written size. */
  const [yieldQuantity, setYieldQuantity] = useState(recipe.yield ?? 1000);
   ...
   * state and the removal list); on success the caller closes the flow, so this
   * component unmounts and only the failure path has state to restore.
   */
      ...
      >
        ...
        <div className="sheet-actions">
          ...
          <button
            ...
            aria-busy={busy}
          >
            {busy ? 'Wird hinzugefügt …' : 'Zum Essensplan hinzufügen'}
          </button>
        </div>
      </div>
    </>
